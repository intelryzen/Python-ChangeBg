...
def filter_skin_and_black(image):
    ...
    lower_skin = np.array([0, 55, 60], dtype=np.uint8)
...
while cap.isOpened():
    ...
    final_result= cv2.cvtColor(color, cv2.COLOR_HSV2BGR)
    ...
    # final_frame = cv2.add(background_part, final_result)
